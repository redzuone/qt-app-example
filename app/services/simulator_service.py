...
class SimulatorService(QObject):
    ...
    def start_simulation(self, target_data: dict[str, Any]) -> None:
        ...
        target_state = _TargetState(
            target_id=target_id,
            target_name=str(
                target_data.get('target_name', f'Target {target_id}')
            ).strip()
    ...
    def _emit_for_targets(self) -> None:
        for target_state in self._targets.values():
            if not target_state.static:
                ...
                turn_noise = (
                    math.sin(target_state.tick / 6.0 + target_state.phased_offset) * 0.8
                )
                ...
                speed_variation = (
                    1.0
                    + math.sin(target_state.tick / 9.0 + target_state.phased_offset)
                    * 0.15
                )
                distance_m = (
                    target_state.speed_mps * max(speed_variation, 0.2) * time_step
                )
                ...
                if target_state.latitude > 89.9:
                    ...
                    target_state.heading_deg = (
                        180.0 - target_state.heading_deg
                    ) % 360.0
